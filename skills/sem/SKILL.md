...
> **Vendored, not ours.** This file is [`skills/sem/SKILL.md`](https://github.com/Ataraxy-Labs/sem/blob/main/skills/sem/SKILL.md)
> from [Ataraxy-Labs/sem](https://github.com/Ataraxy-Labs/sem), at upstream commit `4210e1df5c7b`
> (2026-07-05), under MIT OR Apache-2.0. The body below is upstream's; the
> **In this harness** section at the end is ours. Re-vendor rather than edit upstream's text,
> or the next sync will silently drop the change. `version:` tracks our copy, not the tool.
...
`sem` answers the question this harness keeps getting wrong: *what else does this touch?*

**Run an impact check before you edit a shared function.** Not after, and not instead of reading
the code — before, so the edit is scoped correctly the first time:

```
sem impact <name> --file <path>
```

Three places it earns its keep here:

- **Cross-format blast radius.** The pptx crates share `ooxml-drawingml`, `ooxml-text` and `opc`
  with docx and xlsx. `sem impact arrow --file crates/ooxml-drawingml/src/geometry.rs` reports
  `parse_preset_geometry_path` in `crates/docx-parse/` as a depth-2 dependent — a preset-geometry
  fix is a docx change too. A run shipped exactly that fix without noticing.
- **Before claiming "nothing else calls this".** Agents assert it from a `grep`. `sem impact`
  reports dependents and the tests that cover them, so the claim is checkable and the PR can say
  which tests should have moved.
- **Scoping a cluster before dispatching.** The fix-loop rule is never to give two agents clusters
  that edit the same function. `sem impact` on each candidate's entry point shows the overlap
  before the worktrees exist, rather than at merge time.

Two things verified on this repo, because the invocation is easy to get wrong:

- The entity is **positional**: `sem impact arrow --file path.rs`. There is no `--entity` flag
  (`--entity-id` is a different thing — an id from `sem diff --format json`).
- Paths resolve against the **working tree**, so a file that only exists on another branch reports
  `Path not found`. Check out the branch you mean first.

`sem` is not part of the pipeline: no script calls it, nothing fails without it, and it is listed
in `harness.lock.toml` only because `check_versions.py` accounts for every file on disk. If it is
not installed the agents carry on; they just lose the check.
